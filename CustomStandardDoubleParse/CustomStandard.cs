...
namespace CustomStandardDoubleParse
{
    class CustomStandard
    {
        const string comment = "custom standard: ";
        static double CustomStandardParse(string s)
        {
            string errFrmtMsg = "Входная строка имела не верный формат.";
            ...
            short exp = 0;
            // В стандартном алгоритме
            if (containsExpSymbol && !TryParseExp(s.Substring(expSymbolIndex + 1), out exp))
                throw new FormatException(errFrmtMsg);
            ...
            // Если отсутствуют целая и дробная части, то это ошибка формата
            if (string.IsNullOrEmpty(sAbsIntPart.Trim()) && string.IsNullOrEmpty(sFractionalPart.Trim()))
                throw new FormatException(errFrmtMsg);
            // Если целая часть отстутствует, то она должна быть нулем.
            if (string.IsNullOrEmpty(sAbsIntPart)) sAbsIntPart = "0";
            ...
            if (exp > 308 || exp == 309 && fractionalPart > 17976931348623157)
                throw new OverflowException("Значение было недопустимо малым или недопустимо большим для Double.");
        ...
        static void Main()
        {
            ...
            do
            {
                ...
                //(rnd.NextDouble() * 20 - 10).ToString("g17");
                //                Console.WriteLine(aDoubleString);
                Console.ReadLine();
                // Standard Parse
                try
                {
                ...
                try
                {
                    Console.WriteLine(comment + $"{CustomStandardParse(aDoubleString)}");
                }
                catch (Exception e)
                {
